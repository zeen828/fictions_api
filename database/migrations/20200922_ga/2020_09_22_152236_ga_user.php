...
class GaUser extends Migration
{
    /**
     * Run the migrations.
     * 每小時分析
     * 建立: php artisan make:migration ga_user --path="database/migrations/20200922_ga"
    ...
    public function up()
    {
        Schema::create('t_analysis_user', function (Blueprint $table) {
            ...
            $table->timestamps();
            // 唯一
            $table->unique(['y', 'm', 'd', 'h'], 'hour');
            // 索引
            $table->index(['y', 'm', 'd', 'h', 'date', 'status'], 'query');
        });
    }
    ...
    {
        Schema::dropIfExists('t_analysis_user');
    }
}
